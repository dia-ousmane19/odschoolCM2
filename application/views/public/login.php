  ...
  <form class="form-signin" method="post" action="<?= base_url("login") ?>"> 
    ...
    <?php if (!empty($this->session->userdata('MsgSuccess'))) :?>
      <div class="alert alert-success">
        <?= $this->session->userdata('MsgSuccess') ?>
      </div>
    <?php endif; ?>
    <img class="mb-4" src="<?= base_url("assets/images/logo.png") ?>">
    <h1 class="h3 mb-3 font-weight-normal">Connectez-vous SVP!</h1>
    <input type="text" id="tel_email" class="form-control" name="tel_email" value="<?= set_value('tel_email') ?>" placeholder="Votre email ou votre numéro de téléphone" >
    ...
    Vous n'avez pas encore un compte ? <a href="<?= base_url('sign-up') ?>">Cliquez-ici</a>
